...
function ensureTailwindCSSLoaded() {
    ...
    // The widget utilities are fully namespaced, so always rely on the packaged stylesheet
    // even when the host already ships Tailwind. Skip only when our helper is already present.
    if (typeof window !== 'undefined' && document.getElementById('stiac-accessibility-tailwind')) {
        return;
    }
