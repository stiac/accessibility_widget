...
- Introduced a status bar that lists every active adjustment and lets users quickly disable individual tools from a compact chip row.
- Added a searchable, category-filterable catalog with a favorites rail, collapsible layout controls, and in-depth tool details including live previews.
- Expanded the locale files with translations for the new interface strings so the enhanced experience ships fully localised.
...
- Default the accessibility modal to the bottom-left corner so it aligns with the launcher placement used across most customer deployments while still allowing overrides via `data-default-position` / `data-position`.
...
- Guard the close-button event binding so the scrollbar padding helper always initialises and the modal no longer throws a `ReferenceError` when the launcher node is missing at load time.

## What's New in 1.5.27

- Restore the scrollbar padding helper so opening the accessibility modal no longer triggers a `ReferenceError` and the tools grid stays visually centred.
...
- Updated the launcher so it shows the universal accessibility glyph while the panel is closed and switches to the "X" close icon after opening.

## What's New in 1.5.24

- Updated the close toggle to always render a clear "X" icon so the launcher looks consistent across every alignment and state.

## What's New in 1.5.22

- Restored automatic locale detection by prioritising stored/browser/document languages ahead of defaults and honouring the host page's `<html lang>` attribute.
