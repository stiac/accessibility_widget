...
- Redirected the **Font Dyslexia** toggle to load OpenDyslexic weights directly from the upstream Forge repository so the preference no longer triggers 404s and keeps swap behaviour intact.
...
- Escaped Tailwind's bracketed typography utilities before sampling their computed sizes so the widget no longer raises `querySelector` syntax errors when the modal loads.

## What's New in 1.4.14

- Captured baseline font metrics for the accessibility modal and stored them in CSS variables so the Font Size control now leaves every widget label untouched, even when the host page rescales.

## What's New in 1.4.13

- Counter-scaled Tailwind typography utilities inside `#accessibility-modal` so the Font Size control leaves the widget interface unchanged when zooming the host site.

## What's New in 1.4.12

- Restored the Font Size control to leave the accessibility modal untouched by reapplying original font styles before removing
  registered elements from the scaling map.

## What's New in 1.4.11

- Updated the **Hide Images** toggle to only remove URL-based backgrounds so branded sections such as `#headerContent` keep their gradient or colour treatments while inline media still disappears.

## What's New in 1.4.10

- Prevented the Font Size control from registering the accessibility modal as a resizable target so the widget's own labels remain unchanged while the host site enlarges.

## What's New in 1.4.9

- Restored the browser-default inline behaviour for host audio, canvas, embed, iframe, img, object, svg, and video elements when the fallback Tailwind CDN build is injected while keeping the widget icons block-level for sizing consistency.
...
- Added a dedicated data attribute and CSS variables so the Font Size control reliably enlarges the `<html>` and `<body>` elements while the accessibility modal counter-scales via the stored root value.

## What's New in 1.4.6

- Ensured the font-size control writes scaled pixel values onto the `<html>` and `<body>` elements so the site content grows consistently while the accessibility modal counter-scales via the root variable.
